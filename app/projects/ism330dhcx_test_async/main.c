...
static void
on_ism330dhcx_data(int handle, const void *data, int status, void *ctx)
{
...
main(void)
{
    ...
    board_i2c_add_slave(ISM330DHCX_BUS, ISM330DHCX_SLAVE, ISM330DHCX_ADDR, I2C_ADDR_7BIT,
                        I2C_SPEED_FAST);
    ...
    if (sensor_mgr_subscribe(h, on_ism330dhcx_data, &g_bus.sensors[0]) < 0)
    {
