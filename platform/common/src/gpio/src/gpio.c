...
{
    if(g_gpio.ops->init) g_gpio.ops->init(pin, mode, pp);
}

void gpio_write(gpio_pin_t pin, gpio_level_t level)
{
    if(g_gpio.ops->write) g_gpio.ops->write(pin, level);
}

gpio_level_t gpio_read(gpio_pin_t pin)
{
    if(g_gpio.ops->read) return g_gpio.ops->read(pin);
...
{
    if(g_gpio.ops->toggle) g_gpio.ops->toggle(pin);
}
...
int gpio_irq_register(gpio_pin_t pin,
        ...
        void *ctx)
{
    if (g_gpio.ops->irq_register) return g_gpio.ops->irq_register(pin, edge, cb, ctx);
    return -1;
}

void gpio_irq_enable(gpio_pin_t pin)
{
    if (g_gpio.ops->irq_enable) g_gpio.ops->irq_enable(pin);
}

void gpio_irq_disable(gpio_pin_t pin)
{
    if (g_gpio.ops->irq_disable) g_gpio.ops->irq_disable(pin);
}
