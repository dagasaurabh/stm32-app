...
void gpio_init(gpio_pin_t pin, gpio_mode_t mode, gpio_pull_t pp, gpio_speed_t speed)
{
    ...
    if (g_gpio[ctrl].ops->init) g_gpio[ctrl].ops->init(pin, mode, pp, speed);
}
...
void gpio_write(gpio_pin_t pin, gpio_level_t level)
{
    uint8_t ctrl = gpio_get_ctrl(pin);
    if (ctrl >= GPIO_MAX_CONTROLLERS || !g_gpio[ctrl].initialized) return;
    if (g_gpio[ctrl].ops->write) g_gpio[ctrl].ops->write(pin, level);
}
...
gpio_level_t gpio_read(gpio_pin_t pin)
{
    ...
    if (g_gpio[ctrl].ops->read) return g_gpio[ctrl].ops->read(pin);
    return GPIO_LOW;
}
...
void gpio_toggle(gpio_pin_t pin)
{
    uint8_t ctrl = gpio_get_ctrl(pin);
    if (ctrl >= GPIO_MAX_CONTROLLERS || !g_gpio[ctrl].initialized) return;
    if (g_gpio[ctrl].ops->toggle) g_gpio[ctrl].ops->toggle(pin);
}
        ...
        void *ctx)
{
    ...
    if (g_gpio[ctrl].ops->irq_register) return g_gpio[ctrl].ops->irq_register(pin, edge, cb, ctx);
    return -1;
}
...
void gpio_irq_enable(gpio_pin_t pin)
{
    uint8_t ctrl = gpio_get_ctrl(pin);
    if (ctrl >= GPIO_MAX_CONTROLLERS || !g_gpio[ctrl].initialized) return;
    if (g_gpio[ctrl].ops->irq_enable) g_gpio[ctrl].ops->irq_enable(pin);
}
...
void gpio_irq_disable(gpio_pin_t pin)
{
    uint8_t ctrl = gpio_get_ctrl(pin);
    if (ctrl >= GPIO_MAX_CONTROLLERS || !g_gpio[ctrl].initialized) return;
    if (g_gpio[ctrl].ops->irq_disable) g_gpio[ctrl].ops->irq_disable(pin);
}
