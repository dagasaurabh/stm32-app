...
static int
regmap_spi_read(regmap_t *map, uint8_t reg, uint8_t *buf, uint16_t len);
static int
regmap_spi_write(regmap_t *map, uint8_t reg, const uint8_t *buf, uint16_t len);
static int
regmap_spi_read_async(regmap_t *map, uint8_t reg, uint8_t *buf, uint16_t len, regmap_cb_t cb,
                      void *cb_ctx);
static void
regmap_spi_async_done(void *ctx, spi_evt_t event, uint32_t error_flags);
...
regmap_spi_async_done(void *ctx, spi_evt_t event, uint32_t error_flags)
{
    ...
    if (r->cb)
        r->cb(r->cb_ctx, (event == SPI_EVT_TXRX_DONE) ? 0 : -EIO);
}
