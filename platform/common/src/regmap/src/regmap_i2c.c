 ...
 */
static int
i2c_err_to_errno(uint32_t flags)
{
    if (flags & I2C_ERR_TIMEOUT)
        return -ETIMEDOUT;
    if (flags & I2C_ERR_NACK)
        return -ENXIO;
    if (flags & I2C_ERR_ARLO)
        return -EAGAIN;
    return -EIO;
}

static int
regmap_i2c_read(regmap_t *map, uint8_t reg, uint8_t *buf, uint16_t len);
static int
regmap_i2c_write(regmap_t *map, uint8_t reg, const uint8_t *buf, uint16_t len);
static int
regmap_i2c_read_async(regmap_t *map, uint8_t reg, uint8_t *buf, uint16_t len, regmap_cb_t cb,
                      void *cb_ctx);
static void
regmap_i2c_async_done(void *ctx, i2c_evt_t event, uint32_t error_flags);
                      ...
                      void *cb_ctx)
{
    ...
    r->xfer_reg = (struct i2c_transfer){.dir = I2C_DIR_WRITE, .buf = &r->reg_buf, .len = 1};
    r->xfer_data = (struct i2c_transfer){.dir = I2C_DIR_READ, .buf = buf, .len = len};
