...
struct i2c_bus_ops
{
    int (*open)(void *ctx);
    int (*close)(void *ctx);
    int (*transfer_one)(void *ctx, uint16_t addr, i2c_dir_t dir, uint8_t *buf, uint16_t len,
                        i2c_xfer_opt_t opt);
    int (*transfer_one_it)(void *ctx, uint16_t addr, i2c_dir_t dir, uint8_t *buf, uint16_t len,
 ...
 */
int
i2c_open(const char *slave_name); /* returns fd */
int
i2c_close(int fd);
