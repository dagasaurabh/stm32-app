...
static sensor_entry_t *entry_from_handle(int handle)
{
    if (handle < 0 || handle >= s_mgr.sensor_count) return NULL;
    return &s_mgr.sensor[handle];
}

static void arm_read(sensor_entry_t *e);

static void _on_done(sensor_t *self, int status, void *ctx)
{
    ...
    for (int i = 0; i < e->sub_count; i++)
        e->subs[i].cb(e->handle,
                      status == 0 ? (const void *)e->data_buf : NULL,
                      status,
                      e->subs[i].ctx);
}
...
static void arm_read(sensor_entry_t *e)
{
    ...
    if (sensor_read_async(e->device, _on_done, e) != 0) e->read_pending = false;
}
...
int sensor_mgr_register(sensor_id_t type, sensor_t *device,
        uint32_t interval_ms, uint8_t n_subs)
{
    if (!device)                                    return -EINVAL;
    if (s_mgr.sensor_count >= BOARD_MAX_SENSORS)    return -ENOMEM;
    if (s_mgr.subs_used + n_subs > BOARD_MAX_SUBS)  return -ENOMEM;
...
int sensor_mgr_subscribe(int handle, sensor_data_cb_t cb, void *ctx)
{
    if (!cb) return -EINVAL;
    sensor_entry_t *e = entry_from_handle(handle);
    if (!e)                        return -ENODEV;
    if (e->sub_count >= e->n_subs) return -ENOMEM;
...
int sensor_mgr_unsubscribe(int handle, sensor_data_cb_t cb)
{
    sensor_entry_t *e = entry_from_handle(handle);
...
int sensor_mgr_read_sync(int handle, void *out)
{
    if (!out) return -EINVAL;

    sensor_entry_t *e = entry_from_handle(handle);

    if (!e || !e->device) return -ENODEV;

    return sensor_read_sync(e->device, out);
}

int sensor_mgr_read(int handle)
{
    sensor_entry_t *e = entry_from_handle(handle);

    if (!e || !e->device) return -ENODEV;

    if (e->read_pending)  return -EBUSY;

    arm_read(e);
    return 0;
}

int sensor_mgr_stream_start(int handle)
{
    sensor_entry_t *e = entry_from_handle(handle);
    ...
    if (!e->read_pending) arm_read(e);
    return 0;
}

int sensor_mgr_stream_stop(int handle)
{
    sensor_entry_t *e = entry_from_handle(handle);
    if (!e) return -ENODEV;
    e->streaming = false;
...
int sensor_mgr_reset(int handle)
{
    sensor_entry_t *e = entry_from_handle(handle);
    if (!e) return -ENODEV;
    e->error_count  = 0;
...
void sensor_mgr_poll(void)
{
    ...
    for (int i = 0; i < s_mgr.sensor_count; i++) {
        ...
        if (e->read_pending &&
                (now - e->arm_time_ms) > SENSOR_MGR_TIMEOUT_MS) {

            e->read_pending = false;
            if (++e->error_count >= SENSOR_MGR_MAX_ERRORS) e->streaming = false;
        ...
        /* streaming re-arm */
        if (e->streaming && !e->read_pending &&
           (now - e->arm_time_ms) >= e->interval_ms)
            arm_read(e);
    }
}
...
int  sensor_mgr_register    (sensor_id_t t, sensor_t *d, uint32_t ms, uint8_t n)
                             { (void)t;(void)d;(void)ms;(void)n; return -ENODEV; }
int  sensor_mgr_subscribe   (int h, sensor_data_cb_t cb, void *ctx)
                             { (void)h;(void)cb;(void)ctx; return -ENODEV; }
int  sensor_mgr_unsubscribe (int h, sensor_data_cb_t cb)
                             { (void)h;(void)cb; return -ENODEV; }
int  sensor_mgr_read_sync   (int h, void *out)
                             { (void)h;(void)out; return -ENODEV; }
int  sensor_mgr_read        (int h) { (void)h; return -ENODEV; }
int  sensor_mgr_stream_start(int h) { (void)h; return -ENODEV; }
int  sensor_mgr_stream_stop (int h) { (void)h; return -ENODEV; }
int  sensor_mgr_reset       (int h) { (void)h; return -ENODEV; }
void sensor_mgr_poll        (void)  {}
