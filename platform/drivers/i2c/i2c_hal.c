...
static i2c_hal_slot_t *
s_find_slot(I2C_HandleTypeDef *h)
{
    for (int i = 0; i < HAL_SLOTS; i++)
    {
        if (hal_slots[i].handle == h)
            return &hal_slots[i];
    }
    return NULL;
}
...
static i2c_hal_slot_t *
s_alloc_slot(I2C_HandleTypeDef *h)
{
...
static uint32_t
map_xfer_options(i2c_xfer_opt_t opt)
{
    switch (opt)
    {
        case I2C_XFER_FIRST_AND_LAST:
...
int
i2c_hal_master_tx(void *hal, uint16_t addr, const uint8_t *buf, uint16_t len)
{
    return (HAL_I2C_Master_Transmit((I2C_HandleTypeDef *)hal, addr, (uint8_t *)buf, len,
...
int
i2c_hal_master_rx(void *hal, uint16_t addr, uint8_t *buf, uint16_t len)
{
    return (HAL_I2C_Master_Receive((I2C_HandleTypeDef *)hal, addr, buf, len, HAL_MAX_DELAY) ==
            HAL_OK)
               ? 0
               : -1;
}
 ...
 */
int
i2c_hal_master_seq_tx_it(void *hal, uint16_t addr, uint8_t *buf, uint16_t len, i2c_xfer_opt_t opt,
...
int
i2c_hal_master_seq_rx_it(void *hal, uint16_t addr, uint8_t *buf, uint16_t len, i2c_xfer_opt_t opt,
                         i2c_cb_t cb, void *ctx)
{
    I2C_HandleTypeDef *h = (I2C_HandleTypeDef *)hal;
    i2c_hal_slot_t *s = s_alloc_slot(h);
...
int
i2c_hal_master_seq_tx_dma(void *hal, uint16_t addr, uint8_t *buf, uint16_t len, i2c_xfer_opt_t opt,
                          i2c_cb_t cb, void *ctx)
{
...
int
i2c_hal_master_seq_rx_dma(void *hal, uint16_t addr, uint8_t *buf, uint16_t len, i2c_xfer_opt_t opt,
                          i2c_cb_t cb, void *ctx)
{
