...
int spi_drv_transfer_one(spi_t *dev, const uint8_t *tx, uint8_t *rx, uint16_t len)
{
    ...
    if (tx && rx)  return spi_hal_transfer(dev->hal, tx, rx, len);
    if (tx)        return spi_hal_tx(dev->hal, tx, len);
    if (rx)        return spi_hal_rx(dev->hal, rx, len);
    return -1;
}

int spi_drv_transfer_one_it(spi_t *dev, 
		const uint8_t *tx, uint8_t *rx, uint16_t len, spi_cb_t cb, void *ctx)
{
    if (!dev || !dev->hal || len == 0) return -1;
