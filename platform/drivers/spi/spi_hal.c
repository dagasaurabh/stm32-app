...
static int instance_to_idx(const SPI_TypeDef *inst)
{
    if (inst == SPI1) return 0;
    if (inst == SPI2) return 1;
    if (inst == SPI3) return 2;
    return -1;
}
...
static uint32_t map_hal_error(uint32_t hal_err)
{
    ...
    if (hal_err & HAL_SPI_ERROR_MODF) flags |= SPI_ERR_MODF;
    if (hal_err & HAL_SPI_ERROR_CRC)  flags |= SPI_ERR_CRC;
    if (hal_err & HAL_SPI_ERROR_OVR)  flags |= SPI_ERR_OVR;
    if (hal_err & HAL_SPI_ERROR_FRE)  flags |= SPI_ERR_FRE;
    if (hal_err & HAL_SPI_ERROR_DMA)  flags |= SPI_ERR_DMA;
...
int spi_hal_transfer_it(void *hal, const uint8_t *tx, uint8_t *rx, 
		uint16_t len, spi_cb_t cb, void *ctx)
{
    ...
    if (tx && rx) return HAL_SPI_TransmitReceive_IT(hspi, (uint8_t *)tx, rx, len);
    if (tx)       return HAL_SPI_Transmit_IT(hspi, (uint8_t *)tx, len);
    if (rx)       return HAL_SPI_Receive_IT(hspi, rx, len);
    return -1;
}
...
static void spi_hal_complete(SPI_HandleTypeDef *hspi,
		spi_evt_t event, uint32_t error_flags)
{
    int idx = instance_to_idx(hspi->Instance);
    if (idx < 0) return;
    ...
    if (cb) cb(ctx, event, error_flags);
}
