...
static int
gpio_hal_is_single_bit(uint32_t pin)
{
...
void
gpio_hal_init(void *port, uint32_t pin, gpio_hal_mode_t mode, gpio_pull_t pull,
...
void
gpio_hal_write(void *port, uint32_t pin, uint8_t level)
{
    GPIO_TypeDef *__port = (GPIO_TypeDef *)port;
...
uint8_t
gpio_hal_read(void *port, uint32_t pin)
{
    GPIO_TypeDef *__port = (GPIO_TypeDef *)port;
...
void
gpio_hal_toggle(void *port, uint32_t pin)
{
    GPIO_TypeDef *__port = (GPIO_TypeDef *)port;
...
static uint32_t
exti_line_to_hal(uint32_t line)
{
#define mycase(__int)                                                                              \
    case __int:                                                                                    \
    ...
    switch (line)
    {
        mycase(0) mycase(1) mycase(2) mycase(3) mycase(4) mycase(5) mycase(6) mycase(7) mycase(8)
            mycase(9) mycase(10) mycase(11) mycase(12) mycase(13) mycase(14) mycase(15)
    }
#undef mycase
    return EXTI_LINE_0;
}
...
void
gpio_hal_irq_config(void *port, uint32_t pin, gpio_irq_edge_t edge, gpio_hal_irq_cb_t cb, void *ctx)
{
    GPIO_TypeDef *__port = (GPIO_TypeDef *)port;
...
#undef mycase
    ...
    HAL_EXTI_GetHandle(&hal_exti_slots[line].hexti, exti_line_to_hal(line));
}

static IRQn_Type
exti_line_to_irq(uint32_t line)
{
...
void
gpio_hal_irq_enable(void *port, uint32_t pin)
{
    (void)port;
...
void
gpio_hal_irq_disable(void *port, uint32_t pin)
{
    (void)port;
