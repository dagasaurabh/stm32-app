...
static void hts221_convert(struct hts221_s *dev,
...
static void hts221_regmap_done(void *ctx, int status)
{
 ...
 */
static int hts221_data_ready(struct hts221_s *dev)
{
    uint8_t status = 0;
    if (regmap_read(dev->map, HTS221_REG_STATUS, &status, 1) < 0) return -1;
    return ((status & HTS221_STS_DRDY) == HTS221_STS_DRDY) ? 1 : 0;
}

static int hts221_ops_read_async(sensor_t *self,
        sensor_done_cb_t cb, void *ctx)
{
    ...
    if (rdy < 0)  return -EIO;
    if (rdy == 0) return -EAGAIN;

    dev->read_pending = true;
    dev->pending_cb   = cb;
    dev->pending_ctx  = ctx;
    return regmap_read_async(dev->map, HTS221_REG_H_OUT_L | HTS221_AI,
                             dev->raw, 4, hts221_regmap_done, dev);
}

static int hts221_ops_read_sync(sensor_t *self, void *out)
{
    struct hts221_s *dev  = (struct hts221_s *)self;
    ...
    for (;;) {
        ...
        if (rdy < 0) return -EIO;
        if (rdy > 0) break;
        if (++polls >= HTS221_DRDY_MAX_POLLS) return -ETIMEDOUT;
    }
...
static void hts221_ops_get_data(sensor_t *self, void *out)
{
    struct hts221_s *dev  = (struct hts221_s *)self;
    hts221_data_t   *data = (hts221_data_t *)out;
    hts221_convert(dev, &data->temp_c, &data->humidity_pct);
...
sensor_t *hts221_init(regmap_t *map)
{
    ...
    uint8_t who = 0;
    if (regmap_read(map, HTS221_REG_WHO_AM_I, &who, 1) < 0) return NULL;
    if (who != HTS221_WHOAMI) return NULL;

    uint8_t ctrl = 0x85;
    if (regmap_write(map, HTS221_REG_CTRL1, &ctrl, 1) < 0) return NULL;

    uint8_t cal[16];
    if (regmap_read(map, HTS221_REG_CAL_BASE | HTS221_AI, cal, 16) < 0) return NULL;
