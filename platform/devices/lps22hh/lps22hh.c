...
static int lps22hh_ops_read_async(sensor_t *self,
        sensor_done_cb_t cb, void *ctx)
{
    ...
    return regmap_read_async(dev->map, LPS22HH_REG_PRESS_XL,
                             dev->raw, 5, lps22hh_regmap_done, dev);
}

static int lps22hh_ops_read_sync(sensor_t *self, void *out)
{
    struct lps22hh_s *dev  = (struct lps22hh_s *)self;
    lps22hh_data_t   *data = (lps22hh_data_t *)out;
    if (regmap_read(dev->map, LPS22HH_REG_PRESS_XL, dev->raw, 5) < 0) return -1;
    lps22hh_convert(dev, &data->pressure_hpa, &data->temp_c);
    return 0;
}

static void lps22hh_ops_get_data(sensor_t *self, void *out)
{
    struct lps22hh_s *dev  = (struct lps22hh_s *)self;
    lps22hh_data_t   *data = (lps22hh_data_t *)out;
    lps22hh_convert(dev, &data->pressure_hpa, &data->temp_c);
...
sensor_t *lps22hh_init(regmap_t *map)
{
    ...
    uint8_t who = 0;
    if (regmap_read(map, LPS22HH_REG_WHO_AM_I, &who, 1) < 0) return NULL;
    if (who != LPS22HH_WHOAMI) return NULL;

    uint8_t ctrl = 0x10;
    if (regmap_write(map, LPS22HH_REG_CTRL1, &ctrl, 1) < 0) return NULL;
