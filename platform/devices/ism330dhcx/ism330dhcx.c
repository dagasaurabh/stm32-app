...
static void
ism330dhcx_convert(struct ism330dhcx_s *dev, float *ax, float *ay, float *az, float *gx, float *gy,
                   float *gz)
{
    ...
    if (gx)
        *gx = (float)gxr * ISM330DHCX_GYRO_SENS;
    if (gy)
        *gy = (float)gyr * ISM330DHCX_GYRO_SENS;
    if (gz)
        *gz = (float)gzr * ISM330DHCX_GYRO_SENS;
    if (ax)
        *ax = (float)axr * ISM330DHCX_ACCEL_SENS;
    if (ay)
        *ay = (float)ayr * ISM330DHCX_ACCEL_SENS;
    if (az)
        *az = (float)azr * ISM330DHCX_ACCEL_SENS;
}

static void
ism330dhcx_regmap_done(void *ctx, int status)
{
...
static int
ism330dhcx_ops_read_async(sensor_t *self, sensor_done_cb_t cb, void *ctx)
{
    ...
    dev->pending_cb = cb;
    dev->pending_ctx = ctx;
    return regmap_read_async(dev->map, ISM330DHCX_REG_OUTX_L_G, dev->raw, 12,
                             ism330dhcx_regmap_done, dev);
}

static int
ism330dhcx_ops_read_sync(sensor_t *self, void *out)
{
    struct ism330dhcx_s *dev = (struct ism330dhcx_s *)self;
    ism330dhcx_data_t *data = (ism330dhcx_data_t *)out;
    if (regmap_read(dev->map, ISM330DHCX_REG_OUTX_L_G, dev->raw, 12) < 0)
        return -1;
    ism330dhcx_convert(dev, &data->ax, &data->ay, &data->az, &data->gx, &data->gy, &data->gz);
    return 0;
}

static void
ism330dhcx_ops_get_data(sensor_t *self, void *out)
{
    struct ism330dhcx_s *dev = (struct ism330dhcx_s *)self;
    ism330dhcx_data_t *data = (ism330dhcx_data_t *)out;
    ism330dhcx_convert(dev, &data->ax, &data->ay, &data->az, &data->gx, &data->gy, &data->gz);
...
ism330dhcx_init(regmap_t *map)
{
    ...
    uint8_t who = 0;
    if (regmap_read(map, ISM330DHCX_REG_WHO_AM_I, &who, 1) < 0)
        return NULL;
    if (who != ISM330DHCX_WHOAMI)
        return NULL;

    uint8_t ctrl = 0x40;
    if (regmap_write(map, ISM330DHCX_REG_CTRL1_XL, &ctrl, 1) < 0)
        return NULL;
    if (regmap_write(map, ISM330DHCX_REG_CTRL2_G, &ctrl, 1) < 0)
        return NULL;
