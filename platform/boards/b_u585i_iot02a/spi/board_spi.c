...
static int
board_spi1_transfer_one(void *ctx, const uint8_t *tx, uint8_t *rx, uint16_t len)
{
    return spi_drv_transfer_one((spi_t *)ctx, tx, rx, len);
}

static int
board_spi1_transfer_one_it(void *ctx, const uint8_t *tx, uint8_t *rx, uint16_t len, spi_cb_t cb,
                           void *cb_ctx)
{
...
static void
map_mode(spi_mode_t mode, uint32_t *polarity, uint32_t *phase)
{
    switch (mode)
    {
        case SPI_MODE_0:
...
static uint32_t
map_datasize(spi_datasize_t ds)
{
    switch (ds)
    {
        case SPI_DATASIZE_8:
...
static int
board_spi1_apply_config(void *ctx, spi_mode_t mode, spi_clkdiv_t clkdiv, spi_datasize_t datasize)
{
    spi_t *drv = (spi_t *)ctx;
    ...
    if (phase != hspi->Init.CLKPhase || polarity != hspi->Init.CLKPolarity ||
        prescaler != hspi->Init.BaudRatePrescaler || datawidth != hspi->Init.DataSize)
    {
        spi_drv_deinit(drv);
...
static int
board_spi1_recover(void *ctx, uint32_t error_flags)
{
    spi_t *drv = (spi_t *)ctx;
    SPI_HandleTypeDef *hspi = drv->hal;
 ...
 */
static int
board_spi1_bus_recover(void *ctx)
{
 ...
 *
 * bus_name and name must have static lifetime (pointers are stored directly).
 */
int
board_spi_add_slave(const char *bus_name, const char *name, gpio_pin_t cs_pin, spi_mode_t mode,
                    spi_clkdiv_t prescaler, spi_datasize_t datasize)
{
    if (!bus_name || !name || slave_count >= BOARD_SPI_MAX_SLAVES)
        return -1;
